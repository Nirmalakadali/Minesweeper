...
class MinesweeperAI:
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in neighboring_cells:
            if i in self.mines:
                count_2 -= 1
            if i not in self.mines | self.safes:
                neighboring_cells.add(i)

        n_sentence = Sentence(neighboring_cells, count_2)

        if len(n_sentence.cells) > 0:
            self.knowledge.append(n_sentence)

        self.knowledge_base_loop()

        self.inference()

    def knowledge_base_loop(self):
        know_base = copy.deepcopy(self.knowledge)
        for i in know_base:
            if len(i.cells) == 0:
                try:
    ...
    def inference(self):
        for i, j in itertools.combinations(self.knowledge, 2):
            if i.cells.issubset(j.cells):
                new_cells = j.cells - i.cells
                new_count = j.count - i.count
    ...
    def return_neighbors(self, cell):
        ...
        for i in range(self.height):
            for j in range(self.width):
                if abs(cell[0] - i) <= 1 and abs(cell[1] - j) <= 1 and (i, j) != cell:
